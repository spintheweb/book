...
\begin{description}
\item[\textbf{Webbase Ontology Language (\wbol{})}]: A declarative language for modeling complex portal structures, data relationships, and business logic
\item[\textbf{Web Spinner Runtime}]: A runtime that interprets \wbol{} specifications and delivers dynamic, role-based content
...
\section{Foundations: Webbase Ontology Language (\wbol{})}
\label{sec:blueprint-wbol}

\wbol{} forms the declarative foundation of the entire system. Unlike traditional approaches that mix structure, presentation, and logic, \wbol{} provides a clean separation of concerns through a hierarchical, schema-based approach.
...
\item[\textbf{Technology Agnostic}]: \wbol{} abstracts away implementation details, focusing on business requirements
\end{description}

\subsection{Key Components}

\wbol{} defines several fundamental element types:

\begin{description}
\item[\textbf{STWPortal}]: The root element containing global configuration, datasources, and portal-wide settings
...
\section{Framework: Web Spinner}
\label{sec:engine-spinner}

The Web Spinner runtime serves as the execution layer that brings \wbol{} specifications to life. It handles the complex orchestration of data retrieval, security enforcement, and content delivery that makes dynamic portals possible.

\subsection{Core Responsibilities}

\begin{description}
\item[\textbf{Configuration Interpretation}]: Parsing and optimizing \wbol{} documents for runtime execution
...
\begin{description}
\item[\textbf{Webbase Loader}]: Loads and parses \wbol{} documents into optimized in-memory structures
\item[\textbf{Authorization Module}]: Evaluates role-based visibility rules in real-time
...
\subsection{Role-Based Visibility (Overview)}
\label{sec:rbv-overview}

Spin the Web enforces authorization using a \emph{role-based visibility} (RBV) model. Rather than returning explicit authorization errors, elements not visible to the requester are treated as non-existent and yield a \texttt{200 OK} with an empty response. This privacy-by-design approach applies uniformly to contents, pages, areas, and the portal.

Key characteristics:
\begin{itemize}
	\item \textbf{Declarative rules in \wbol{}}: visibility is defined per element and can be inherited.
	\item \textbf{Inheritance}: when a role setting is \texttt{undefined}, visibility is inherited recursively from the parent up to the root.
	\item \textbf{Three-state roles}: \texttt{true} (granted), \texttt{false} (denied), \texttt{undefined} (inherit).
	\item \textbf{Safe defaults}: unauthenticated users receive the \texttt{guests} role by default; elements are invisible unless made visible via rules or inheritance.
\end{itemize}

For operational details see \secref{sec:security} in the Web Spinner chapter, and for the formal schema see the \wbol{} appendix (Appendix~\ref{app:wbol-xml-schema}).

\paragraph{Side note: \texttt{rwx} and RBV}
The familiar UNIX-style \texttt{rwx} access paradigm can be expressed with RBV: a content that supports reading, writing, or executing (in any combination) can be exposed to specific roles. In practice, "read" aligns with viewing content, while "write" and "execute" align with content operations (e.g., mutations or actions). RBV governs \emph{visibility} of these capabilities per role; the concrete enforcement of operation-level permissions is handled at runtime by the Web Spinner security layer (see \secref{sec:security}).
...
\section{Portal: Spin the Web Studio}
...
\item[\textbf{Code Assistance}]: Intelligent editing support for \wbol{} and \wbpl{} syntax with multi-tab file editing capabilities
...
\begin{itemize}
	\item \textbf{Full Control}: Complete access to all \wbol{} features and capabilities
...
\begin{enumerate}
	...
	\item \wbol{} documents are saved and versioned using standard development practices
	\item The Web Spinner runtime loads updated \webbase{} definitions and applies changes
...
\begin{enumerate}
	\item User requests arrive at the Web Spinner runtime
	\item The runtime consults the in-memory \webbase{} structure to route the request
	\item Authorization rules defined in \wbol{} are evaluated against user roles
...
\begin{enumerate}
	\item Start with core portal structure and basic navigation
