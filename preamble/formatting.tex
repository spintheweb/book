...
% Hyperlink setup: keep only metadata here; link options live in packages.tex
\hypersetup{
    pdftitle={Spin the Web},
    pdfauthor={Giancarlo Trevisan (\organization)},
    pdfsubject={Enterprise Web Portal Development},
    pdfkeywords={WBOL, Web Spinner, Web Portal, Enterprise Software Integrator, STW}
}

% Cross-reference style: use § before section/chapter numbers
% Applies to \cref and \Cref for single and plural/ranged references
...
% Define XML language (used for WBOL listings)
...
\lstdefinestyle{jsonstyle}{
    ...
    morestring=[b]",
    stringstyle=\color{stringcolor},
    morekeywords={true,false,null,_id,type,name,slug,keywords,description,visibility,children,mainpage,langs,datasources,version,namespace,rules,role,visible,subtype,command,layout,options,connectionString,baseUrl},
