...
\section{Integration with WBOL}
\label{sec:wbpl-integration}

\wbpl{} is primarily used within \texttt{STWContent} elements in \wbol{} documents. The \texttt{command} attribute of an \texttt{STWContent} element contains a template string that is processed by the \wbpl{} engine before being executed against the specified datasource.
...
\item Global variables (portal configuration, system settings)
...
While \wbol{} provides the structure and \wbpl{} injects dynamic data, a critical question remains: how do we manage the complexity of a large-scale enterprise portal? How do we ensure that different parts of the portal can be developed, maintained, and deployed independently without causing system-wide disruptions?
