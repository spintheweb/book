...
\wbol{} is formally defined using the standard \textbf{JSON Schema}.
...
\wbol{} defines a base element, \texttt{STWElement}, from which all other elements inherit. JSON Schema definitions for \wbol{} elements are in \Cref{app:wbol-json-schema} (see Appendix: WBOL JSON Schema Reference).
...
The \texttt{datasources} element within \texttt{STWSite} provides a flexible framework for defining how the portal connects to and manages external data systems. Beyond simple connection definitions, \wbol{} supports advanced data organization concepts:
