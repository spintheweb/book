...
This appendix summarizes three foundational webbaselets that commonly ship with a Spin the Web deployment. Each is delivered as a standalone webbaselet (root \texttt{webbaselet} element) that can be imported into any Webbase, sharing common conventions for navigation, security, and data.
...
A minimal root for each webbaselet as a \texttt{webbaselet} element:
\begin{lstlisting}[language=XML,caption={Minimal Webbaselet Root}]
<webbaselet xmlns="https://spintheweb.org/schemas/wbol.xsd"
    guid="aBPMSRoot000000001" version="1.0.0">
  <name><text xml:lang="en" slug="bpms">BPMS</text></name>
</webbaselet>
\end{lstlisting}
...
To make webbaselets discoverable and governable across the ecosystem, the root \texttt{webbaselet} of each SHOULD declare a compact, structured \emph{classification}. This is a proposed extension to the base \wbol{} XSD (Appendix~\ref{app:wbol-xml-schema}): a dedicated \texttt{classification} element, distinct from \texttt{name}/\texttt{visibilities}/\texttt{documentation}, to avoid collisions with core elements. Engines MAY also surface these as indexed metadata for search.
...
\subsection{Example}
BPMS classification on its root \texttt{webbaselet}:
\begin{lstlisting}[language=XML,caption={Webbaselet with classification}]
<webbaselet xmlns="https://spintheweb.org/schemas/wbol.xsd"
    guid="aBPMSRoot000000001" version="1.0.0">
  <name><text xml:lang="en" slug="bpms">BPMS</text></name>
  <classification purpose="bpms" domain="hr" layer="ui"
      criticality="high" maturity="stable" visibility="authenticated"
      dataSensitivity="pii">
    <capability>workflow</capability>
    <capability>approvals</capability>
    <capability>forms</capability>
    <capability>sla</capability>
    <audience>employees</audience>
    <audience>managers</audience>
    <audience>admins</audience>
    <compliance>gdpr</compliance>
    <i18n>en</i18n>
    <i18n>it</i18n>
    <engine min="1.3.0" />
    <dependsOn>directory&gt;=1.1</dependsOn>
    <dependsOn>notifications</dependsOn>
    <imports>tickets.created</imports>
    <exports>bpms.task.created</exports>
    <exports>bpms.task.completed</exports>
    <tags>process</tags>
    <tags>tasks</tags>
...
\paragraph{Notes}
\begin{itemize}
  \item Engines should index \texttt{classification} attributes/children for search and dependency checks.
  \item Recommending enumerated attribute values keeps catalogs consistent; use \texttt{tags} for additional nuance.
...
These values map to \texttt{classification} attributes and inform facets in catalog pages.
